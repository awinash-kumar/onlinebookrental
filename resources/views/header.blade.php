<style>
.modal-backdrop.show {
    opacity: .5;
    z-index: 1;
}
</style>
<nav class="navbar" style="background: antiquewhite;">
    <div class="navbar-container">
        <input type="checkbox" name="" id="checkbox">
        <ul class="menu-items">
            <li><a href="{{ route('dashboard')}}">Home</a></li>
            <li><a href="{{route('about_us')}}" >About Us</a></li>
            <li><a href="{{route('paidorder.index')}}">My order</a></li>
            <li><a href="{{ route('contact_us')}}">Contact</a></li>

        </ul>
        <div class="logo">
            <img class="books_logo"
                src="https://thumbs.dreamstime.com/b/hand-book-logo-illustration-art-background-43965136.jpg" width="70"
                height="70" style="border-radius: 48px;" alt="">
        </div>
    </div>
    <div class="ct-section">
        @php
        use App\Models\RentCard;
        use Illuminate\Support\Facades\Auth;
        $userId = Auth::id();
        $totaladdcart = RentCard::where('user_id', $userId)->where('book_status',1)->count();
        @endphp
        <div>
            <a href="{{ route('addcart') }}">
                <i class="fa fa-cart-plus" style="font-size:37px;"><sup><strong>{{ $totaladdcart }}</strong></sup></i>
            </a>
        </div>
        <div
            style="text-align: center;display: flex;flex-direction: column;align-items: center;justify-content: center;">
            <div style="height: 44px;width:44px; ">
                <img style="height:100%;width:100%;object-fit:cover; border-radius:22px"
                    src="data:image/png;base64,"
                    alt="">
            </div>
            <div class="" style="font-size: 12px; color:grey"><strong>{{ Auth::user()->name }}</strong></div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <x-responsive-nav-link :href="route('logout')" onclick="event.preventDefault();
                                        this.closest('form').submit();">
                    <!-- <i class="fa fa-sign-"></i> <button type="submit" style="background: #8f4646;"
                    class="font-semibold border sm:border-2 h-8 sm:h-10 px-3 sm:px-4 rounded text-sm sm:text-base text-white hover:text-purple-400 border-white hover:bg-white">
                    {{ __('Log Out') }}</button> -->
                    <i class="fa fa-sign-out" style="color: grey;"></i>
                </x-responsive-nav-link>
            </form>
        </div>
    </div>
</nav>